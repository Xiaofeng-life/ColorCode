...
class PairDataset(data.Dataset):
    def __init__(self, images_path, labels_path, img_size, if_train, trans_hazy=None, trans_gt=None,
        ...
        self.trans_resize = tt.Resize((self.img_size[0], self.img_size[1]))
    ...
    def __getitem__(self, index):
        ...
        data_mask = Image.open(os.path.join(self.clear_dir, clear_name).replace("labels", "mask"))
        data_mask = torch.from_numpy(np.array(data_mask)).float()
        # data_mask = self.trans_resize(data_mask)
        data_mask[data_mask <= 20] = 0
        data_mask[data_mask > 20] = 1
        if len(data_mask.size()) == 2:
            data_mask = data_mask.unsqueeze(2)
            data_mask = data_mask.repeat(1, 1, 3)
        data_mask = data_mask.permute(2, 0, 1)
        data_mask = self.trans_resize(data_mask)
        ...
        tar_data = {"blur": data_hazy, "gt": data_gt,  "mask": data_mask,
...
class PairDataset_NoMask(data.Dataset):
    def __init__(self, images_path, labels_path, img_size, if_train, trans_hazy=None, trans_gt=None,
                 if_identity_name=False):
        super().__init__()
        self.haze_imgs_dir = os.listdir(images_path)
...
def get_train_val_loader(data_train, data_val, train_batch_size, num_workers, if_mask):
    try:
    ...
    if if_mask == "Y":
        ...
        train_dataset = PairDataset_NoMask(images_path=os.path.join(data_root_train, "images/"),
                                    labels_path=os.path.join(data_root_train, "labels/"),
                                    img_size=[256, 256], if_train=True,
                                    trans_hazy=None, trans_gt=None, if_identity_name=True)

        test_dataset = PairDataset_NoMask(images_path=os.path.join(data_root_val, "images/"),
                                   labels_path=os.path.join(data_root_val, "labels/"),
                                   img_size=[256, 256], if_train=False,
                                   trans_hazy=None, trans_gt=None, if_identity_name=True)
